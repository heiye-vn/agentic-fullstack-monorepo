 ...
 *
 * 需求分析多 Agent 编排流水线中 5 个 Agent 的 ChatPromptTemplate 定义。
 * 每个导出为一个完整、立即可用的 Agent 提示词模板（不含 model 和 parser）。
 */
import {
  ChatPromptTemplate,
  MessagesPlaceholder,
...
// Agent 1：extractAgent — 从用户输入抽取结构化需求字段（输出纯 JSON）
...
// Agent 2：clarifyAgent — 判断是否需要澄清并生成问题（输出纯 JSON）
...
// Agent 3：analysisAgent — 多维度需求分析（输出 Markdown）
...
// Agent 4：riskAgent — 风险识别与评估（输出 Markdown，与 analysisAgent 并行执行）
...
// Agent 5：summaryAgent — 生成最终需求分析报告（输出完整 Markdown 报告）
