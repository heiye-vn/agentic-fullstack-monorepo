...
async function doRefresh(): Promise<void> {
  ...
  if (!refreshToken) {
    ...
    throw new Error('本地无 refreshToken，需重新登录');
  }

  try {
    // Use bare axios (not the wrapped instance) to avoid interceptor recursion.
    // 注意：agentic 的 user-system 没有全局 ResponseInterceptor，返回的是裸对象
    //   { accessToken, refreshToken, expiresIn, user }
    // 而 autix 那边是包了一层的
    //   { success, code, msg, traceId, data: { accessToken, refreshToken } }
    // 这里两种形状都要兼容，否则 tokens 会是 undefined，刷新直接失败并把人踢回登录页。
    const refreshRes = await axios.post<ApiResponse<{ accessToken: string; refreshToken: string }>>(
      `${USER_API}/auth/refresh`,
      { refreshToken },
    );
    const body = refreshRes.data as any;
