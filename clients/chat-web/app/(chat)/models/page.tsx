...
export default function ModelsPage() {
  ...
  useEffect(() => {
    ...
    getAvailableModels()
      .then(({ data }) => {
        if (!ignore) {
          setModels(data as ModelConfigItem[]);
        }
      })
      .catch(() => {})
      .finally(() => {
                  ...
                  <div className="flex items-center gap-3 pt-1">
                    <button
                      ...
                      className={`relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                        editing.isDefault ? 'bg-orange-600' : 'bg-neutral-700'
                      }`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out ${
                          editing.isDefault ? 'translate-x-5' : 'translate-x-0'
                        }`}
                      />
                    </button>
                    <span
                      onClick={() => setEditing({ ...editing, isDefault: !editing.isDefault })}
                      className="text-xs text-neutral-300 select-none cursor-pointer hover:text-white transition-colors"
                    >
                      设为默认模型
                    </span>
                  </div>
